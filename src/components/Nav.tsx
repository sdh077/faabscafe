...
export default function Nav({ name, brand, navItems }: { name: string, brand: { img?: string, title: string }, navItems: NavItem[] }) {
    ...
    return (
        <Navbar className='fixed w-full top-0' fluid rounded style={{ boxShadow: '0 11px 24px rgba(15,19,33,.04)' }} >
            <NavbarBrand href="/">
                {brand?.img && <img src={brand.img} className="mr-3 h-[20px] w-[165px] sm:h-[20px]" alt="Flowbite React Logo" />}
            ...
            <NavbarCollapse>
                {navItems.map(navItem =>
                    ...
                    //     </NavbarLink>
                    //     :
                    <NavbarLink key={navItem.name} href={`/${navItem.link}`} active={navItem.link === pathname}>
                        {navItem.name}
                    </NavbarLink>
                )}
                {!name &&
